...
type IPv6PrefixLists struct {
	Id     types.String                    `tfsdk:"id"`
	Domain types.String                    `tfsdk:"domain"`
	Items  map[string]IPv6PrefixListsItems `tfsdk:"items"`
}

type IPv6PrefixListsItems struct {
	Id      types.String                  `tfsdk:"id"`
	Type    types.String                  `tfsdk:"type"`
	Entries []IPv6PrefixListsItemsEntries `tfsdk:"entries"`
}

type IPv6PrefixListsItemsEntries struct {
...
var minFMCVersionBulkCreateIPv6PrefixLists = version.Must(version.NewVersion("999"))
var minFMCVersionBulkDeleteIPv6PrefixLists = version.Must(version.NewVersion("999"))

// End of section. //template:end minimumVersions

// Section below is generated&owned by "gen/generator.go". //template:begin getPath

func (data IPv6PrefixLists) getPath() string {
...
func (data IPv6PrefixLists) toBody(ctx context.Context, state IPv6PrefixLists) string {
	body := ""
...
func (data *IPv6PrefixLists) fromBody(ctx context.Context, res gjson.Result) {
	...
	for k := range data.Items {
		...
		if value := res.Get("entries"); value.Exists() {
			data.Entries = make([]IPv6PrefixListsItemsEntries, 0, len(value.Array()))
...
func (data *IPv6PrefixLists) fromBodyPartial(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Items {
		...
		}
		{
			...
			for i := len(data.Entries); i < l; i++ {
				data.Entries = append(data.Entries, IPv6PrefixListsItemsEntries{})
			}
...
func (data *IPv6PrefixLists) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
...
func (data *IPv6PrefixLists) Clone() IPv6PrefixLists {
	ret := *data
...
func (data IPv6PrefixLists) toBodyNonBulk(ctx context.Context, state IPv6PrefixLists) string {
...
func (data IPv6PrefixLists) adjustBody(ctx context.Context, req string) string {
