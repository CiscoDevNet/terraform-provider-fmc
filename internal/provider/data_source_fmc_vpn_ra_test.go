...
func TestAccDataSourceFmcVPNRA(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_vpn_ra.test", "description", "My Remote Access VPN Configuration"))
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_vpn_ra.test", "web_access_port", "443"))
...
const testAccDataSourceFmcVPNRAPrerequisitesConfig = `
...
resource "fmc_ikev2_policies" "test" {
  items = {
    "ravpn_ikev2_policy1" = {
      description           = "ravpn_ikev2_policy"
      priority              = 44
      lifetime              = 28800
      encryption_algorithms = ["AES-192", "AES-256"]
      integrity_algorithms  = ["SHA", "SHA-256"]
      prf_algorithms        = ["SHA", "SHA-256"]
      dh_groups             = ["14", "15"]
    }
  }
}
...
func testAccDataSourceFmcVPNRAConfig() string {
	...
	config += `	description = "My Remote Access VPN Configuration"` + "\n"
	...
	config += `		id = fmc_ikev2_policies.test.items.ravpn_ikev2_policy1.id` + "\n"
	config += `	}]` + "\n"
	config += `}` + "\n"
