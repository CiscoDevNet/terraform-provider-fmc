...
func TestAccDataSourceFmcFTDPlatformSettingsICMPAccess(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_ftd_platform_settings_icmp_access.test", "icmp_configs.0.action", "Permit"))
...
const testAccDataSourceFmcFTDPlatformSettingsICMPAccessPrerequisitesConfig = `
...
func testAccDataSourceFmcFTDPlatformSettingsICMPAccessConfig() string {
	...
	config += `
		data "fmc_ftd_platform_settings_icmp_access" "test" {
			id = fmc_ftd_platform_settings_icmp_access.test.id
