...
type FTDPlatformSettingsICMPAccess struct {
	...
	Configurations        []FTDPlatformSettingsICMPAccessConfigurations `tfsdk:"configurations"`
}
...
type FTDPlatformSettingsICMPAccessConfigurationsInterfaceObjects struct {
	Id   types.String `tfsdk:"id"`
	Type types.String `tfsdk:"type"`
...
func (data *FTDPlatformSettingsICMPAccess) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("icmpConfigs"); value.Exists() {
		data.Configurations = make([]FTDPlatformSettingsICMPAccessConfigurations, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			...
			if value := res.Get("interfaces.objects"); value.Exists() {
				data.InterfaceObjects = make([]FTDPlatformSettingsICMPAccessConfigurationsInterfaceObjects, 0)
				value.ForEach(func(k, res gjson.Result) bool {
					parent := &data
...
func (data *FTDPlatformSettingsICMPAccess) fromBodyPartial(ctx context.Context, res gjson.Result) {
	...
	for i := 0; i < len(data.Configurations); i++ {
		...
		if !res.Exists() {
			tflog.Debug(ctx, fmt.Sprintf("removing Configurations[%d] = %+v",
				i,
				(*parent).Configurations[i],
			))
			(*parent).Configurations = slices.Delete((*parent).Configurations, i, i+1)
			i--
