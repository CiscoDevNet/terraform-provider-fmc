...
type DistinguishedNames struct {
	...
	Items  map[string]DistinguishedNamesItems `tfsdk:"items"`
}

type DistinguishedNamesItems struct {
	Id                types.String `tfsdk:"id"`
...
var minFMCVersionDistinguishedNames = version.Must(version.NewVersion("7.4"))
var minFMCVersionBulkCreateDistinguishedNames = version.Must(version.NewVersion("999"))
var minFMCVersionBulkDeleteDistinguishedNames = version.Must(version.NewVersion("999"))

// End of section. //template:end minimumVersions

// Section below is generated&owned by "gen/generator.go". //template:begin getPath

func (data DistinguishedNames) getPath() string {
...
func (data DistinguishedNames) toBody(ctx context.Context, state DistinguishedNames) string {
	body := ""
	...
	if len(data.Items) > 0 {
		...
		for key, item := range data.Items {
			...
			itemBody, _ = sjson.Set(itemBody, "type", "DistinguishedName")
			if !item.DistinguishedName.IsNull() {
				itemBody, _ = sjson.Set(itemBody, "dn", item.DistinguishedName.ValueString())
			}
			if itemBody != "" {
				if itemsBody.Len() > 1 {
					itemsBody.WriteString(",")
...
func (data *DistinguishedNames) fromBody(ctx context.Context, res gjson.Result) {
...
func (data *DistinguishedNames) fromBodyPartial(ctx context.Context, res gjson.Result) {
...
func (data *DistinguishedNames) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
...
func (data *DistinguishedNames) Clone() DistinguishedNames {
	ret := *data
...
func (data DistinguishedNames) toBodyNonBulk(ctx context.Context, state DistinguishedNames) string {
