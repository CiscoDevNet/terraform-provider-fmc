...
func TestAccDataSourceFmcSLAMonitor(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_sla_monitor.test", "name", "my_sla_monitor"))
	checks = append(checks, resource.TestCheckResourceAttrSet("data.fmc_sla_monitor.test", "type"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_sla_monitor.test", "description", "My SLA Monitor object"))
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_sla_monitor.test", "monitor_address", "10.10.10.1"))
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceFmcSLAMonitorPrerequisitesConfig + testAccDataSourceFmcSLAMonitorConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
			{
				Config: testAccDataSourceFmcSLAMonitorPrerequisitesConfig + testAccNamedDataSourceFmcSLAMonitorConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
		},
	})
...
const testAccDataSourceFmcSLAMonitorPrerequisitesConfig = `
...
func testAccDataSourceFmcSLAMonitorConfig() string {
	...
	config += `
		data "fmc_sla_monitor" "test" {
			id = fmc_sla_monitor.test.id
		}
	`
	return config
}

func testAccNamedDataSourceFmcSLAMonitorConfig() string {
	config := `resource "fmc_sla_monitor" "test" {` + "\n"
	config += `	name = "my_sla_monitor"` + "\n"
	config += `	description = "My SLA Monitor object"` + "\n"
	config += `	sla_monitor_id = 10` + "\n"
	...
	config += `
		data "fmc_sla_monitor" "test" {
			name = fmc_sla_monitor.test.name
		}
	`
	return config
}
