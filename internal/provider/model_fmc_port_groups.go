...
type PortGroupsItems struct {
	...
	Ports       []PortGroupsItemsPorts `tfsdk:"ports"`
}

type PortGroupsItemsPorts struct {
	Id   types.String `tfsdk:"id"`
	Type types.String `tfsdk:"type"`
}
...
func (data PortGroups) getPath() string {
...
func (data PortGroups) toBody(ctx context.Context, state PortGroups) string {
	body := ""
	...
	if len(data.Items) > 0 {
		...
		for key, item := range data.Items {
			...
			if len(item.Ports) > 0 {
				...
				for _, childItem := range item.Ports {
					itemChildBody := ""
...
func (data *PortGroups) fromBody(ctx context.Context, res gjson.Result) {
	for k := range data.Items {
		...
		if value := res.Get("objects"); value.Exists() {
			data.Ports = make([]PortGroupsItemsPorts, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := PortGroupsItemsPorts{}
				if value := res.Get("id"); value.Exists() {
					data.Id = types.StringValue(value.String())
				} else {
					data.Id = types.StringNull()
...
func (data *PortGroups) fromBodyPartial(ctx context.Context, res gjson.Result) {
...
func (data *PortGroups) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
