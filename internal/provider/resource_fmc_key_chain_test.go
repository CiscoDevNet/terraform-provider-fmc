...
func TestAccFmcKeyChain(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("fmc_key_chain.test", "name", "my_key_chain"))
	...
	checks = append(checks, resource.TestCheckResourceAttr("fmc_key_chain.test", "keys.0.send_lifetime_start", "2025-08-25T12:14:23"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_key_chain.test", "keys.0.send_lifetime_end_type", "DATETIME"))
	checks = append(checks, resource.TestCheckResourceAttr("fmc_key_chain.test", "keys.0.send_lifetime_end", "2026-08-25T12:14:23"))

	var steps []resource.TestStep
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccFmcKeyChainConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccFmcKeyChainConfig_all(),
		Check:  resource.ComposeTestCheckFunc(checks...),
	})
	steps = append(steps, resource.TestStep{
		ResourceName: "fmc_key_chain.test",
		ImportState:  true,
	})
...
func testAccFmcKeyChainConfig_all() string {
	config := `resource "fmc_key_chain" "test" {` + "\n"
	config += `	name = "my_key_chain"` + "\n"
