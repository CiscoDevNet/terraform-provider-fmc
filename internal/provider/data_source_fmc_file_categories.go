		...
		Attributes: map[string]schema.Attribute{
			...
			"items": schema.MapNestedAttribute{
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						"id": schema.StringAttribute{
							MarkdownDescription: "Id of the File Category.",
							Computed:            true,
						},
...
func (d *FileCategoriesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if d.client.FMCVersionParsed.LessThan(minFMCVersionFileCategories) {
	...
	if len(config.Items) == 0 {
		if config.Items == nil {
			config.Items = map[string]FileCategoriesItems{}
		}
		res.Get("items").ForEach(func(_, v gjson.Result) bool {
			if name := v.Get("name").String(); name != "" {
				config.Items[name] = FileCategoriesItems{}
			}
			return true
		})
	}
