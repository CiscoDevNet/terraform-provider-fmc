...
func TestAccDataSourceFmcIPv6PrefixLists(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceFmcIPv6PrefixListsConfig(),
...
func testAccDataSourceFmcIPv6PrefixListsConfig() string {
	...
	config += `
		data "fmc_ipv6_prefix_lists" "test" {
			depends_on = [fmc_ipv6_prefix_lists.test]
			items = {
