...
type MACAddressPools struct {
	Id     types.String                    `tfsdk:"id"`
	Domain types.String                    `tfsdk:"domain"`
	Items  map[string]MACAddressPoolsItems `tfsdk:"items"`
}

type MACAddressPoolsItems struct {
	Id              types.String `tfsdk:"id"`
	Type            types.String `tfsdk:"type"`
	Description     types.String `tfsdk:"description"`
	MacAddressRange types.String `tfsdk:"mac_address_range"`
...
func (data MACAddressPools) getPath() string {
	return "/api/fmc_config/v1/domain/{DOMAIN_UUID}/object/macaddresspools"
}
...
func (data MACAddressPools) toBody(ctx context.Context, state MACAddressPools) string {
	body := ""
	...
	if len(data.Items) > 0 {
		...
		for key, item := range data.Items {
			...
			itemBody, _ = sjson.Set(itemBody, "type", "MacAddressPool")
			if !item.Description.IsNull() {
				itemBody, _ = sjson.Set(itemBody, "description", item.Description.ValueString())
			}
			if !item.MacAddressRange.IsNull() {
				itemBody, _ = sjson.Set(itemBody, "macAddressRange", item.MacAddressRange.ValueString())
			}
...
func (data *MACAddressPools) fromBody(ctx context.Context, res gjson.Result) {
	...
	for k := range data.Items {
		...
		if value := res.Get("macAddressRange"); value.Exists() {
			data.MacAddressRange = types.StringValue(value.String())
		} else {
			data.MacAddressRange = types.StringNull()
		}
...
func (data *MACAddressPools) fromBodyPartial(ctx context.Context, res gjson.Result) {
	...
	for i := range data.Items {
		...
		if value := res.Get("macAddressRange"); value.Exists() && !data.MacAddressRange.IsNull() {
			data.MacAddressRange = types.StringValue(value.String())
		} else {
			data.MacAddressRange = types.StringNull()
		}
...
func (data *MACAddressPools) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
...
func (data *MACAddressPools) Clone() MACAddressPools {
	ret := *data
...
func (data MACAddressPools) toBodyNonBulk(ctx context.Context, state MACAddressPools) string {
