...
func TestAccDataSourceFmcCertificateEnrollment(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_certificate_enrollment.test", "name", "my_certificate_enrollment"))
	checks = append(checks, resource.TestCheckResourceAttrSet("data.fmc_certificate_enrollment.test", "type"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_certificate_enrollment.test", "description", "My certificate enrollment"))
...
const testAccDataSourceFmcCertificateEnrollmentPrerequisitesConfig = `
locals {
  certificate = <<EOT
...
func testAccDataSourceFmcCertificateEnrollmentConfig() string {
	config := `resource "fmc_certificate_enrollment" "test" {` + "\n"
	config += `	name = "my_certificate_enrollment"` + "\n"
	config += `	description = "My certificate enrollment"` + "\n"
	config += `	enrollment_type = "PKCS12"` + "\n"
	config += `	pkcs12_certificate = base64encode(local.certificate)` + "\n"
	config += `	pkcs12_certificate_passphrase = "cisco123"` + "\n"
	config += `}` + "\n"

	config += `
		data "fmc_certificate_enrollment" "test" {
			id = fmc_certificate_enrollment.test.id
		}
	`
	return config
}

func testAccNamedDataSourceFmcCertificateEnrollmentConfig() string {
	config := `resource "fmc_certificate_enrollment" "test" {` + "\n"
	config += `	name = "my_certificate_enrollment"` + "\n"
	config += `	description = "My certificate enrollment"` + "\n"
	config += `	enrollment_type = "PKCS12"` + "\n"
	...
	config += `
		data "fmc_certificate_enrollment" "test" {
			name = fmc_certificate_enrollment.test.name
		}
	`
	return config
}
