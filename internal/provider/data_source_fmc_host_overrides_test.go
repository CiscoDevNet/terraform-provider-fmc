...
const testAccDataSourceFmcHostOverridesPrerequisitesConfig = `
...
func testAccDataSourceFmcHostOverridesConfig() string {
	...
	config += `
		data "fmc_host_overrides" "test" {
			id = fmc_host_overrides.test.id
		}
	`
	return config
}
...
func testAccNamedDataSourceFmcHostOverridesConfig() string {
	...
	config += `
		data "fmc_host_overrides" "test" {
			parent_name = fmc_host_overrides.test.parent_name
		}
	`
	return config
}
