...
func TestAccDataSourceFmcTunnelZone(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceFmcTunnelZoneConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
			{
				Config: testAccNamedDataSourceFmcTunnelZoneConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
		},
	})
...
func testAccDataSourceFmcTunnelZoneConfig() string {
	...
	config += `
		data "fmc_tunnel_zone" "test" {
			id = fmc_tunnel_zone.test.id
		}
	`
	return config
}
...
func testAccNamedDataSourceFmcTunnelZoneConfig() string {
	...
	config += `
		data "fmc_tunnel_zone" "test" {
			name = fmc_tunnel_zone.test.name
		}
	`
	return config
}
