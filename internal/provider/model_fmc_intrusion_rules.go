...
type IntrusionRules struct {
	Id     types.String                   `tfsdk:"id"`
	Domain types.String                   `tfsdk:"domain"`
	Items  map[string]IntrusionRulesItems `tfsdk:"items"`
}

type IntrusionRulesItems struct {
	Id         types.String                    `tfsdk:"id"`
	RuleData   types.String                    `tfsdk:"rule_data"`
	RuleGroups []IntrusionRulesItemsRuleGroups `tfsdk:"rule_groups"`
	Type       types.String                    `tfsdk:"type"`
}

type IntrusionRulesItemsRuleGroups struct {
	Id   types.String `tfsdk:"id"`
...
var minFMCVersionBulkDeleteIntrusionRules = version.Must(version.NewVersion("999"))

// End of section. //template:end minimumVersions

// Section below is generated&owned by "gen/generator.go". //template:begin getPath

func (data IntrusionRules) getPath() string {
...
func (data IntrusionRules) toBody(ctx context.Context, state IntrusionRules) string {
	body := ""
	...
	if len(data.Items) > 0 {
		...
		for key, item := range data.Items {
			...
			if !item.RuleData.IsNull() {
				itemBody, _ = sjson.Set(itemBody, "ruleData", item.RuleData.ValueString())
			}
			if len(item.RuleGroups) > 0 {
				itemBody, _ = sjson.Set(itemBody, "ruleGroups", []any{})
				for _, childItem := range item.RuleGroups {
					itemChildBody := ""
					...
					itemBody, _ = sjson.SetRaw(itemBody, "ruleGroups.-1", itemChildBody)
				}
			}
...
func (data *IntrusionRules) fromBody(ctx context.Context, res gjson.Result) {
	...
	for k := range data.Items {
		...
		if value := res.Get("ruleGroups"); value.Exists() {
			data.RuleGroups = make([]IntrusionRulesItemsRuleGroups, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := IntrusionRulesItemsRuleGroups{}
				if value := res.Get("id"); value.Exists() {
					data.Id = types.StringValue(value.String())
				} else {
					data.Id = types.StringNull()
...
func (data *IntrusionRules) fromBodyPartial(ctx context.Context, res gjson.Result) {
...
func (data *IntrusionRules) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
...
func (data *IntrusionRules) Clone() IntrusionRules {
	ret := *data
...
func (data IntrusionRules) toBodyNonBulk(ctx context.Context, state IntrusionRules) string {
