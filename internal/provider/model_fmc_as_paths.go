...
type ASPaths struct {
	...
	Items  map[string]ASPathsItems `tfsdk:"items"`
}

type ASPathsItems struct {
	Id          types.String          `tfsdk:"id"`
	Type        types.String          `tfsdk:"type"`
	Overridable types.Bool            `tfsdk:"overridable"`
	Entries     []ASPathsItemsEntries `tfsdk:"entries"`
}

type ASPathsItemsEntries struct {
...
var minFMCVersionBulkCreateASPaths = version.Must(version.NewVersion("999"))
var minFMCVersionBulkDeleteASPaths = version.Must(version.NewVersion("999"))

// End of section. //template:end minimumVersions

// Section below is generated&owned by "gen/generator.go". //template:begin getPath

func (data ASPaths) getPath() string {
...
func (data ASPaths) toBody(ctx context.Context, state ASPaths) string {
	body := ""
...
func (data *ASPaths) fromBody(ctx context.Context, res gjson.Result) {
	...
	for k := range data.Items {
		...
		if value := res.Get("entries"); value.Exists() {
			data.Entries = make([]ASPathsItemsEntries, 0, len(value.Array()))
...
func (data *ASPaths) fromBodyPartial(ctx context.Context, res gjson.Result) {
...
func (data *ASPaths) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
...
func (data *ASPaths) Clone() ASPaths {
	ret := *data
...
func (data ASPaths) toBodyNonBulk(ctx context.Context, state ASPaths) string {
...
func (data ASPaths) adjustBody(ctx context.Context, req string) string {
