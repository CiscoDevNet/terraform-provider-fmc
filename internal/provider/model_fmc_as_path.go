...
func (data ASPath) toBody(ctx context.Context, state ASPath) string {
	body := ""
...
func (data *ASPath) fromBody(ctx context.Context, res gjson.Result) {
	...
	if value := res.Get("entries"); value.Exists() {
		data.Entries = make([]ASPathEntries, 0, len(value.Array()))
...
func (data *ASPath) fromBodyPartial(ctx context.Context, res gjson.Result) {
	...
	}
	{
		...
		for i := len(data.Entries); i < l; i++ {
			data.Entries = append(data.Entries, ASPathEntries{})
		}
...
func (data *ASPath) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
