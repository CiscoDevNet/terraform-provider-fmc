...
func TestAccDataSourceFmcExpandedCommunityList(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceFmcExpandedCommunityListConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
			{
				Config: testAccNamedDataSourceFmcExpandedCommunityListConfig(),
				Check:  resource.ComposeTestCheckFunc(checks...),
			},
		},
	})
...
func testAccDataSourceFmcExpandedCommunityListConfig() string {
	...
	config += `
		data "fmc_expanded_community_list" "test" {
			id = fmc_expanded_community_list.test.id
		}
	`
	return config
}

func testAccNamedDataSourceFmcExpandedCommunityListConfig() string {
	config := `resource "fmc_expanded_community_list" "test" {` + "\n"
	config += `	name = "my_expanded_community_list"` + "\n"
	config += `	entries = [{` + "\n"
	config += `		action = "PERMIT"` + "\n"
	...
	config += `
		data "fmc_expanded_community_list" "test" {
			name = fmc_expanded_community_list.test.name
		}
	`
	return config
}
