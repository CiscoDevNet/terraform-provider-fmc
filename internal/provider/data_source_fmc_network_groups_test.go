...
func TestAccDataSourceFmcNetworkGroups(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttrSet("data.fmc_network_groups.test", "items.my_network_groups.id"))
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_network_groups.test", "items.my_network_groups.description", "My Network Group 1"))
	checks = append(checks, resource.TestCheckResourceAttrSet("data.fmc_network_groups.test", "items.my_network_groups.type"))
	checks = append(checks, resource.TestCheckResourceAttrSet("data.fmc_network_groups.test", "items.my_network_groups.objects.0.id"))
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_network_groups.test", "items.my_network_groups.literals.0.value", "10.1.1.0/24"))
...
func testAccDataSourceFmcNetworkGroupsConfig() string {
	...
	config += `
		data "fmc_network_groups" "test" {
			depends_on = [fmc_network_groups.test]
			items = {
				"my_network_groups" = {
				}
			}
