...
func TestAccDataSourceFmcFTDPlatformSettingsSyslogSettingsSyslogID(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.fmc_ftd_platform_settings_syslog_settings_syslog_id.test", "syslog_id", "305011"))
...
func testAccDataSourceFmcFTDPlatformSettingsSyslogSettingsSyslogIDConfig() string {
	...
	config += `	syslog_id = 305011` + "\n"
	config += `	log_level = "ERR"` + "\n"
	config += `	enabled = true` + "\n"
	config += `}` + "\n"

	config += `
		data "fmc_ftd_platform_settings_syslog_settings_syslog_id" "test" {
			id = fmc_ftd_platform_settings_syslog_settings_syslog_id.test.id
			ftd_platform_settings_id = fmc_ftd_platform_settings.test.id
			ftd_platform_settings_syslog_settings_id = fmc_ftd_platform_settings_syslog_settings.test.id
		}
	`
	return config
}
