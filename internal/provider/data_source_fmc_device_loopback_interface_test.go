...
func testAccDataSourceFmcDeviceLoopbackInterfaceConfig() string {
	...
	config += `
		data "fmc_device_loopback_interface" "test" {
			id = fmc_device_loopback_interface.test.id
...
func testAccNamedDataSourceFmcDeviceLoopbackInterfaceConfig() string {
	config := `resource "fmc_device_loopback_interface" "test" {` + "\n"
	config += `	device_id = var.device_id` + "\n"
	config += `	logical_name = "my_loopback_1"` + "\n"
	config += `	enabled = true` + "\n"
