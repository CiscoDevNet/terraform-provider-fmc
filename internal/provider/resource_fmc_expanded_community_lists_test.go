...
func TestAccFmcExpandedCommunityLists(t *testing.T) {
	...
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccFmcExpandedCommunityListsConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccFmcExpandedCommunityListsConfig_all(),
...
func testAccFmcExpandedCommunityListsConfig_minimum() string {
...
func testAccFmcExpandedCommunityListsConfig_all() string {
	config := `resource "fmc_expanded_community_lists" "test" {` + "\n"
	config += `	items = { "my_expanded_community_lists" = {` + "\n"
	config += `		entries = [{` + "\n"
	config += `			action = "PERMIT"` + "\n"
