...
func (data Ranges) toBody(ctx context.Context, state Ranges) string {
	body := ""
...
func (data *Ranges) fromBody(ctx context.Context, res gjson.Result) {
	for k := range data.Items {
		...
		if value := res.Get("type"); value.Exists() {
			...
			data.Type = types.StringValue("Range")
		}
		(*parent).Items[k] = data
	}
}
...
func (data *Ranges) fromBodyPartial(ctx context.Context, res gjson.Result) {
	for i := range data.Items {
		...
		if value := res.Get("type"); value.Exists() && !data.Type.IsNull() {
			data.Type = types.StringValue(value.String())
		} else if data.Type.ValueString() != "Range" {
...
func (data *Ranges) fromBodyUnknowns(ctx context.Context, res gjson.Result) {
