...
type DeviceBulk struct {
	Devices []Device `json:"devices"`
}

type DeviceBulkResponse struct {
	DeviceResponses []DeviceResponse `json:"devicesresponses"`
}
...
func (v *Client) CreateFmcDeviceBulk(ctx context.Context, devices *DeviceBulk) (*DeviceBulkResponse, error) {
	url := fmt.Sprintf("%s/devices/devicerecords?bulk=true", v.domainBaseURL)
	body, err := json.Marshal(devices.Devices)
	if err != nil {
...
func (v *Client) DeleteFmcDeviceBulk(ctx context.Context, ids string) error {
	url := fmt.Sprintf("%s/devices/devicerecords?filter:ids=%s", v.domainBaseURL, ids)
	req, err := http.NewRequestWithContext(ctx, "DELETE", url, nil)
	if err != nil {
		return fmt.Errorf("deleting Device: %s - %s", url, err.Error())
	}
